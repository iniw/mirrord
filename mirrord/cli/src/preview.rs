...
async fn create_preview_target(
    ...
    // Determine the namespace for the preview CRD.
    // Priority: CLI arg > config target namespace > "default"
    let namespace = args
        .target_namespace
        .as_deref()
    ...
    let spec = PreviewTargetSpec {
        ...
        target: config.target.path.clone(),
        target_namespace: args.target_namespace.clone(),
        ttl_seconds: args.ttl,
        agent_image: Some(config.agent.image().to_string()),
        agent_log_level: Some(config.agent.log_level.clone()),
    };
...
fn display_preview_info(preview: &PreviewTargetCrd, key: &str, progress: &mut ProgressTracker) {
    let namespace = preview.namespace().unwrap_or_else(|| "default".to_string());
    let name = preview.name_any();
    ...
    let ttl = preview.spec.ttl_seconds;
...
use the same key with 'mirrord exec --key {key}' to connect to this preview.
"#
    )));
}
