...
pub(super) struct ExecParams {
    ...
    /// If not provided here or in the config file, a UUID is generated automatically.
    #[arg(long)]
...
pub(super) struct PreviewStartArgs {
    ...
    /// Target to copy configuration from (optional).
    ///
    /// Valid formats:
    /// - `pod/{pod-name}[/container/{container-name}]`
    /// - `deployment/{deployment-name}[/container/{container-name}]`
    ...
    /// TTL in seconds for the preview Job.
    ///
    /// After this time, the preview pod will be automatically deleted.
...
impl PreviewStartArgs {
    ...
    pub fn as_env_vars(&self) -> HashMap<&'static OsStr, &OsStr> {
        ...
        if let Some(key) = &self.key {
            envs.insert("MIRRORD_ENV_KEY".as_ref(), key.as_ref());
        }
        if let Some(target) = &self.target {
            envs.insert("MIRRORD_IMPERSONATED_TARGET".as_ref(), target.as_ref());
        }
