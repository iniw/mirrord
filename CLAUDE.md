...
calls (files, networking, env) and turns them into protocol messages sent to intproxy. Manages local fd and socket state.
- **`mirrord-intproxy`**: sits between the layer(s) and the agent. Matches requests to responses, handles reconnects,
and routes messages to the right feature proxy (files, incoming, outgoing, DNS, env).
- **`mirrord-agent`**: runs in the Kubernetes cluster in the target pod's context. Does the actual remote work: file
operations, network mirroring/stealing, outgoing connections, DNS resolution, env var extraction.
...
(sent by the layer/intproxy to the agent) and `DaemonMessage` (sent by the agent back). These are serialized with
